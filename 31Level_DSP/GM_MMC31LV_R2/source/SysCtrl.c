...
void Init_PLL(void)
{
    ...
    if(ClkCfgRegs.CLKSRCCTL1.bit.OSCCLKSRCSEL != 1)
    {
        ...
        ClkCfgRegs.CLKSRCCTL1.bit.OSCCLKSRCSEL = 1;         // Xtal 20MHz
    }

    ClkCfgRegs.SYSPLLCTL1.bit.PLLCLKEN = 0;                 // Bypass the PLL by clearing SYSPLLCTL1[PLLCLKEN]
    ClkCfgRegs.SYSCLKDIVSEL.bit.PLLSYSCLKDIV = 0;           // Set the system Clock divider t0 /1 for the fastest PLL configuration

    //Lock the PLL 5 times
    for(ui_temp = 0; ui_temp < 5; ui_temp++)
    {
        ClkCfgRegs.SYSPLLMULT.bit.FMULT = 0;                // Set fractional multiplier
        ClkCfgRegs.SYSPLLMULT.bit.IMULT = 20;               // Set integral multiplier 20MHz(Xtal)*(IMULT+FMULT) / PLLSYSCLKDIV

        while(ClkCfgRegs.SYSPLLSTS.bit.LOCKS != 1);
    }

    ClkCfgRegs.SYSCLKDIVSEL.bit.PLLSYSCLKDIV = 2;           // Set the system clock divider one setting higher thant final value
    DevCfgRegs.SYSDBGCTL.bit.BIT_0 = 1;                     // Set the SYSDBGCTL[BIT_0]

    while (ClkCfgRegs.X1CNT.bit.X1CNT < 0x3FF);             // X1 Counter. Saturated with 0x3FF Value.
    ClkCfgRegs.SYSPLLCTL1.bit.PLLCLKEN = 1;                 // Set SYSPLLCTL[PLLCLKEN]

    DevCfgRegs.SYSDBGCTL.bit.BIT_0 = 0;                     // Set the SYSDBGCTL[BIT_0]

    ClkCfgRegs.SYSCLKDIVSEL.bit.PLLSYSCLKDIV = 1;           // Set the system clock divider to final value(1: /2 -> 400Mhz/2 = 200Mhz)
...
void Init_PieVectTable(void)
{
	...
	PieVectTable.EPWM1_INT = &ISR_ePWM1_CPU1_MMC_OP;			// 3.1 - ePWM1 Interrupt
    ...
    PieVectTable.EPWM2_INT = &ISR_ePWM2_CPU2_MMC_OP;            // 3.2 - ePWM2 Interrupt
    PieVectTable.EPWM3_INT = &ISR_Default;                      // 3.3 - ePWM3 Interrupt
    PieVectTable.EPWM4_INT = &ISR_Default;                      // 3.4 - ePWM4 Interrupt
    PieVectTable.EPWM5_INT = &ISR_Default;                      // 3.5 - ePWM5 Interrupt
    PieVectTable.EPWM6_INT = &ISR_Default;                      // 3.6 - ePWM6 Interrupt
...
{
#ifdef CPU1
    PieCtrlRegs.PIEIER3.bit.INTx1 = 1;                  //PIE Group3, channel 1 : EPWM1
	IER |= M_INT3;                                      //CPU Interrupt 3 IER
#endif

#ifdef CPU2
    PieCtrlRegs.PIEIER3.bit.INTx2 = 1;                  //PIE Group3, channel 2 : EPWM2
    IER |= M_INT3;                                      //CPU Interrupt 3 IER
#endif

    // CPUx Master Interrupt Enable.
    EINT;                                               // Master Interrupt Enable(INTM).
