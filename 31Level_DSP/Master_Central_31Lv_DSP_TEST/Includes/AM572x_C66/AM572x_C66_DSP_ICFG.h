...
union PDCCMD_REG {
    Uint32                  all;
    struct PDCCMD_BITS      bit;
};

struct MM_REVID_BITS {
    Uint32      rsvd0:16;
    Uint32      REVID:16;
};
...
union MM_REVID_REG {
    Uint32                  all;
    struct MM_REVID_BITS    bit;
};

struct IDMA_STAT_BITS {
    Uint32      rsvd0:30;
    Uint32      PEND:1;
    Uint32      ACTV:1;
};
...
union IDMA_STAT_REG {
    Uint32                  all;
    struct IDMA_STAT_BITS   bit;
};

struct IDMA_MASK_BITS {
    Uint32      M0:1;
    Uint32      M1:1;
    Uint32      M2:1;
    Uint32      M3:1;
...
union IDMA_MASK_REG {
    Uint32                  all;
    struct IDMA_MASK_BITS   bit;
};

struct IDMA_SOURCE_BITS {
    Uint32      SOURCEADDR:27;
    Uint32      rsvd0:5;
};

union IDMA_SOURCE_REG {
    Uint32                  all;
    struct IDMA_SOURCE_BITS bit;
};

struct IDMA_DEST_BITS {
    Uint32      DESTADDR:27;
    Uint32      rsvd0:5;
};
...
union IDMA_DEST_REG {
    Uint32                  all;
    struct IDMA_DEST_BITS   bit;
};

struct IDMA_COUNT_BITS {
    Uint32      rsvd0:3;
    Uint32      INT:1;
    Uint32      rsvd1:24;
    Uint32      COUNT:4;
...
union IDMA_COUNT_REG {
    Uint32                  all;
    struct IDMA_COUNT_BITS  bit;
};
...
struct DSP_ICFG_REGS {								// Offset
	...
	/*
	Uint32                      rsvd18_C;           // 0x018C
	Uint32                      rsvd19[4];          // 0x0190
	Uint32                      rsvd1A[4];          // 0x01A0
	Uint32                      rsvd1B[4];          // 0x01B0
	Uint32                      EVTASRT;            // 0x01C0
	Uint32                      rsvd1C[3];          // 0x01C4
	Uint32                      rsvd1D[4];          // 0x01D0
	Uint32                      rsvd1E[4];          // 0x01E0

	Uint32                      rsvd;

	union PDCCMD_REG            PDCCMD;             // 0x1 0000

	Uint32                      rsvd;

	union MM_REVID_REG          MM_REVID;           // 0x1 2000

	Uint32                      rsvd;

	union IDMA_STAT_REG         IDMA0_STAT;         // 0x2 0000
	union IDMA_MASK_REG         IDMA0_MASK;         // 0x2 0004
	union IDMA_SOURCE_REG       IDMA0_SOURCE;       // 0x2 0008
	union IDMA_DEST_REG         IDMA0_DEST;         // 0x2 000C
	union IDMA_COUNT_REG        IDMA0_COUNT;        // 0x2 0010
	Uint32                      rsvd;               //
    union IDMA_STAT_REG         IDMA1_STAT;         // 0x2 0100
    Uint32                      rsvd;               // 0x2 0104
    union IDMA_SOURCE_REG       IDMA1_SOURCE;       // 0x2 0108
    union IDMA_DEST_REG         IDMA1_DEST;         // 0x2 010C
    union IDMA_COUNT_REG        IDMA1_COUNT;        // 0x2 0110
    */
};
